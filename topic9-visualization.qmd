...
res[[:xmin, :xmax, :ymin, :ymax]] # x and y limits
res[:node_data]  # info to add node annotations
res[:edge_data]  # info to add edge annotations

# find order "o" in which species from data are listed in the network -- and in res
...
tips = res[:node_data][!,:lea]
dat.morph[o] == res[:node_data][tips,:name] # should be true

# next: add grey & red points at the leaves
tip_col = map(x -> (x=="lo" ? "grey" : "red"), dat.elevation[o])
R"points"(
    x=res[:node_data][tips,:x] .+0.1,
    y=res[:node_data][tips,:y],
...
R"legend"(x=1, y=19, legend=["hi","lo"], pch=16,
    ...
    bty="n", var"title.adj"=0);
    # var"" to allow for a dot in the variable name: R's fault...

# next: add γ's for gene flow edges
hybminor = res[:edge_data].min
hybdf = res[:edge_data][hybminor, [:x,:y,:gam]]
R"text"(hybdf.x .- 0.1, hybdf.y + 0.5.*[-1,1,1], hybdf.gam,
...
ndf = res[:node_data] # had locations of internal nodes, this time
...
o = [findfirst(isequal(tax), traits.morph) for tax in res[:node_data].name] # ordering
intervallength = 2
...
x0 = res[:node_data][1,:x] + 0.1 # x where the smallest bar should extend
xmean = x0 - m           # x where the mean value will go
# create the bars
R"segments"(x0 = xmean, x1 = xmean .+ std_elev,
            y0=res[:node_data].y, col=linecol,
...
asr_larea_01 = standardize(UnitRangeTransform, asr_larea.prediction) # values in 0-1
@rlibrary viridis
...
# find order to match tips between asr_larea and res[:node_data]
ndf = res[:node_data]
...
# add legend
m,M = string.(round.(extrema(asr_larea.prediction), digits=2))
